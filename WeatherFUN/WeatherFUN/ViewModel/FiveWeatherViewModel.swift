...
class FiveWeatherViewModel: ObservableObject {
    ...
    func fetchCurrentWeatherData() {
        ...
        let nextdayTime = Array(fiveDayWeatherDates[6].time) //둘째날의 일을 가져옴
        ...
        for d in fiveDayWeatherDates {
            ...
            if (todayTime[9] == t[9] && String(t[11]) + String(t[12]) >= formattedDate){ //첫째날의 일과 같으면서 사용자의 시간보다 이미 지난 시간은 버려서 배열에 저장
